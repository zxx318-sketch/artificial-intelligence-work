...
class Tensor:
    ...
    def im2col(self, kernel_size: int, stride: int) -> Tensor:
        return Im2Col.apply(self, tensor(kernel_size), tensor(stride))
    ...
    def expand(self, other: Tensor) -> Tensor:
        ...
        # Case 3: Still different (broadcast in forward pass), reduce extra dims.
        out = buf

        # For better alignment, we add "1"s to the front of self.shape until it is
        # the same length as out.shape. For example, if self.shape is (3,) and
        # out.shape is (2, 3), we want to treat self.shape as (1, 3) for the
        # purposes of broadcasting.
        orig_shape = [1] * (len(out.shape) - len(self.shape)) + list(self.shape)

        # Sum out all broadcasted dimensions.
        for dim, (o, t) in enumerate(zip(out.shape, orig_shape)):
            if t == 1 and o != 1:
    ...
    def __hash__(self) -> int:
        return hash(self.unique_id)  # 假设 unique_id 是整数且唯一
